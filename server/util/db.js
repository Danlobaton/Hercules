...
module.exports.check_if_current = function(view, object_id, getData) {
  let sql = `SELECT * FROM Facebook_Ads.CampaignDBreakdown WHERE CampaignID = ${object_id} AND Date = DATE_SUB(CURDATE(), INTERVAL 1 DAY);`;
  try {
    con.query(sql, function(err, result) {
      if(err) {
        throw err;
...
module.exports.get_last_date = function(view, object_id, getData) {
  let sql = `SELECT DATE_FORMAT(Date, "%y-%m-%d") FROM Facebook_Ads.CampaignDBreakdown WHERE CampaignID = ${object_id} ORDER BY Date DESC LIMIT 1;`;
  try {
      con.query(sql, function(err, result) {
      if(err) {
        throw err;
...
module.exports.campaign_current = function(camp_id, getData) {
  let sql = `SELECT Purchases, DATE_FORMAT(Date, "%y-%m-%d") FROM Facebook_Ads.CampaignDBreakdown WHERE CampaignID = ${camp_id} ORDER BY Date ASC LIMIT 60;`;
  try {
    con.query(sql, function(err, result) {
      if (err)
        throw err;
